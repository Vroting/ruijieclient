...
GType ruijie_dbus_get_type (void);
...
static void ruijie_dbus_sig_StateChanged (RuijieDbus* self, DBusGProxy* obj_, guint32 i);
static void _ruijie_dbus_sig_StateChanged_dynamic_StateChanged0_ (DBusGProxy* _sender, guint32 i, gpointer self);
void _dynamic_StateChanged1_connect (gpointer obj, const char * signal_name, GCallback handler, gpointer data);
void ruijie_dbus_conn_dbus (RuijieDbus* self, GError** error);
static inline guint32 _dynamic_get_State0 (DBusGProxy* obj);
guint32 ruijie_dbus_get_state (RuijieDbus* self);
RuijieDbus* ruijie_dbus_new (void);
RuijieDbus* ruijie_dbus_construct (GType object_type);
static void ruijie_dbus_finalize (GObject* obj);
static void _lambda0_ (Block1Data* _data1_);
static void __lambda0__ruijie_dbus_sig_p_statechanged (RuijieDbus* _sender, gpointer self);
...
void _dynamic_StateChanged1_connect (gpointer obj, const char * signal_name, GCallback handler, gpointer data) {
	dbus_g_object_register_marshaller (g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, G_TYPE_UINT, G_TYPE_INVALID);
...
void ruijie_dbus_conn_dbus (RuijieDbus* self, GError** error) {
	...
	self->priv->networkmanager = (_tmp2_ = dbus_g_proxy_new_for_name (self->priv->conn, "org.freedesktop.NetworkManager", "/org/freedesktop/NetworkManager", "org.freedesktop.NetworkManager"), _g_object_unref0 (self->priv->networkmanager), _tmp2_);
	_dynamic_StateChanged1_connect (self->priv->networkmanager, "StateChanged", (GCallback) _ruijie_dbus_sig_StateChanged_dynamic_StateChanged0_, self);
}
...
static void ruijie_dbus_class_init (RuijieDbusClass * klass) {
	...
	g_signal_new ("sig_p_statechanged", TYPE_RUIJIE_DBUS, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}


static void ruijie_dbus_instance_init (RuijieDbus * self) {
...
GType ruijie_dbus_get_type (void) {
	...
	if (ruijie_dbus_type_id == 0) {
		static const GTypeInfo g_define_type_info = { sizeof (RuijieDbusClass), (GBaseInitFunc) NULL, (GBaseFinalizeFunc) NULL, (GClassInitFunc) ruijie_dbus_class_init, (GClassFinalizeFunc) NULL, NULL, sizeof (RuijieDbus), 0, (GInstanceInitFunc) ruijie_dbus_instance_init, NULL };
		ruijie_dbus_type_id = g_type_register_static (G_TYPE_OBJECT, "RuijieDbus", &g_define_type_info, 0);
	}
...
gint connect_to_sig_StateChanged (Fuction fast, void* fast_target) {
	...
	g_signal_connect_data (dbus, "sig-p-statechanged", (GCallback) __lambda0__ruijie_dbus_sig_p_statechanged, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	result = 0;
	block1_data_unref (_data1_);
	return result;
}
